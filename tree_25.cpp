...
void topViewHelper(Btree<int>* root,map<int,pair<int,int> > &m,int hd,int level)
{
    if(root==NULL)
    return;
    if(m.find(hd)==m.end())
    ...
    topViewHelper(root->left,m,hd-1,level+1);
    topViewHelper(root->right,m,hd+1,level+1);
}
void topView(Btree<int> *root)
{
   // Your Code Here
   map<int,pair<int,int> >m;
   topViewHelper(root,m,0,0);
...
int main()
{
    ...
    cout<<"the top view of a tree is "<<endl;
    topView(root);
    cout<<endl;
}
